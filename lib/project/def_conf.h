...
#define MODELName           "EPaperDesperta"            // Hardware model name (TTGoTS = ZENDesperta, T5 = ZENDespertaE)
...
#define PBUZZER             33                          // (Passive) Buzzer pin. Suggest to use pin 18. -1 means NOT used!
...
#define BUT_A               39                          // Button A INPUT pin (used in buttons.h) TTGoTS=35, T5=39
...
#define T_Left              T9                          // Touch button Left  pin. TTGoTS=T7, T5=-1, -1 means NOT used!
...
#define Default_ADC_PIN     35                          // IO pin forBattery ADC measurement. TFT = 36, EPaper = 35
...
#define CLK_PIN             18                          // SPI CLK pin, Display TTGoTS-> 5, T5->18, -1 means NOT used!
#define CS_PIN               5                          // SPI CS pin, Display  TTGoTS->16, t5->5 -1 means NOT used!
...
#define BUSY_PIN             4                          // Display BUSY pin,  TTGoTS->-1, T5->4  -1 means NOT used!


void config_defaults() {
    Serial.println("Setting config Default values");

    strcpy(config.DeviceName, "EPaperDesperta");          // Device Name
    ...
    config.Volume = 75;                                   // Speaker volume [0-100%].
    config.Alarm_State = true;                            // Alarm state (true -> Ring / False -> Not Ring)
    config.AlarmDateTime = {6, 40, 0, 0, 0, 0, 9, true, 1};// Alarm DateTime structure
}
